...
export class AppEvent extends Event {
  ...
  playlistHistoryUpdated(history: LX.Player.PlaylistHistoryItem[]) {
    this.emit('playlistHistoryUpdated', history)
  }

  playHistoryUpdated(history: LX.Player.PlayHistoryItem[]) {
    this.emit('playHistoryUpdated', history)
  }
