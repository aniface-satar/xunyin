...
export const sanitizeMusicInfo = (musicInfo: LX.Music.MusicInfo): LX.Music.MusicInfo => {
  const info: AnyRecord = { ...musicInfo }
  const meta: AnyRecord = isRecord(info.meta) ? { ...info.meta } : {}
  const source = info.source ?? (typeof meta.filePath == 'string' && meta.filePath ? 'local' : 'kw')

  info.id = info.id == null ? '' : String(info.id)
  info.name = info.name == null ? '' : String(info.name)
  info.singer = info.singer == null ? '' : String(info.singer)
  info.interval = info.interval == null ? null : String(info.interval)
  info.source = source

  meta.songId = meta.songId ?? info.id
  meta.albumName = meta.albumName == null ? '' : String(meta.albumName)
  meta.picUrl = typeof meta.picUrl == 'string' ? meta.picUrl : null

  if (source == 'local') {
    if (meta.filePath == null) meta.filePath = typeof meta.songId == 'string' ? meta.songId : info.id
    meta.ext = typeof meta.ext == 'string' ? meta.ext : (/\.(\w+)$/.exec(meta.filePath as string)?.[1] ?? '')
  } else {
    if (!Array.isArray(meta.qualitys)) meta.qualitys = []
    if (!isRecord(meta._qualitys)) meta._qualitys = {}
  }

  info.meta = meta
  return info as LX.Music.MusicInfo
}

export const sanitizeMusicList = (list: unknown): LX.Music.MusicInfo[] => {
  if (!Array.isArray(list)) return []
  return list.filter(isRecord).map(info => sanitizeMusicInfo(info as LX.Music.MusicInfo))
}
...
export const fixNewMusicInfoQuality = (musicInfo: LX.Music.MusicInfo) => {
  const info = sanitizeMusicInfo(musicInfo)
  if (info.source == 'local') return info

  const qualitys = info.meta._qualitys as Record<string, any> | undefined
  if (qualitys?.flac32bit && !qualitys.flac24bit) {
    qualitys.flac24bit = qualitys.flac32bit
    delete qualitys.flac32bit

    info.meta.qualitys = info.meta.qualitys.map(quality => {
      // @ts-expect-error Old data may contain the removed quality key
      if (quality.type == 'flac32bit') quality.type = 'flac24bit'
      return quality
    })
  }
