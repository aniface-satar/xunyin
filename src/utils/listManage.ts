...
import { sanitizeMusicList } from '@/utils'
...
const updateList = ({
  ...
  switch (id) {
    ...
    default:
      ...
      if (index < 0) return
      userLists.splice(index, 1, { ...userLists[index], name, source, sourceListId, isLove, locationUpdateTime })
      break
  }
}
...
export const userListCreate = ({ name, id, source, sourceListId, isLove, position, locationUpdateTime }: {
  name: string
  id: string
