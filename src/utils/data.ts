...
const playlistHistoryKey = storageDataPrefix.playlistHistory
...
let playlistHistory: LX.Player.PlaylistHistoryItem[] | undefined
let playlistHistoryLoadPromise: Promise<LX.Player.PlaylistHistoryItem[]> | undefined
let playlistHistoryUpdatePromise = Promise.resolve()
let playHistory: LX.Player.PlayHistoryItem[] | undefined
let playHistoryLoadPromise: Promise<LX.Player.PlayHistoryItem[]> | undefined
let playHistoryUpdatePromise = Promise.resolve()
...
const savePlaylistHistoryThrottle = throttle(() => {
  void saveData(playlistHistoryKey, playlistHistory)
}, 1000)
const savePlayHistoryThrottle = throttle(() => {
  void saveData(playHistoryKey, playHistory)
}, 1000)
...
const loadPlaylistHistory = async() => {
  if (playlistHistory) return [...playlistHistory]
  playlistHistoryLoadPromise ??= getData<LX.Player.PlaylistHistoryItem[]>(playlistHistoryKey).then(list => list ?? [])
  // eslint-disable-next-line require-atomic-updates
  playlistHistory = await playlistHistoryLoadPromise
  // eslint-disable-next-line require-atomic-updates
  playlistHistoryLoadPromise = undefined
  return [...playlistHistory]
}

export const getPlaylistHistory = async() => {
  return loadPlaylistHistory()
}

export const savePlaylistHistory = async(info: Omit<LX.Player.PlaylistHistoryItem, 'playTime'> & { playTime?: number }) => {
  if (!info.listId || !info.source || !info.sourceListId || !info.name) return

  const update = async() => {
    await loadPlaylistHistory()
    const listId = info.sourceListId.startsWith('board__')
      ? info.sourceListId
      : `${info.source}__${info.sourceListId}`
    playlistHistory = [
      { ...info, listId, playTime: info.playTime ?? Date.now() },
      ...playlistHistory!.filter(item => item.source !== info.source || item.sourceListId !== info.sourceListId),
    ].slice(0, 50)

    savePlaylistHistoryThrottle()
    global.app_event.playlistHistoryUpdated([...playlistHistory])
  }
  playlistHistoryUpdatePromise = playlistHistoryUpdatePromise.then(update, update)
  return playlistHistoryUpdatePromise
}

const loadPlayHistory = async() => {
  if (playHistory) return [...playHistory]
  playHistoryLoadPromise ??= getData<LX.Player.PlayHistoryItem[]>(playHistoryKey).then(list => list ?? [])
  // eslint-disable-next-line require-atomic-updates
  playHistory = await playHistoryLoadPromise
  // eslint-disable-next-line require-atomic-updates
  playHistoryLoadPromise = undefined
  return [...playHistory]
}
...
const getPlayHistoryKey = (item: Pick<LX.Player.PlayHistoryItem, 'listId' | 'musicInfo'>) => {
  return `${item.listId ?? ''}__${item.musicInfo.id ?? ''}`
}

export const addPlayHistory = async(info: LX.Player.PlayMusicInfo) => {
  const item: LX.Player.PlayHistoryItem = {
    musicInfo: info.musicInfo,
    listId: info.listId,
    isTempPlay: info.isTempPlay,
    playTime: Date.now(),
  }
  const update = async() => {
    await loadPlayHistory()
    const key = getPlayHistoryKey(item)
    playHistory = [
      item,
      ...playHistory!.filter(historyItem => getPlayHistoryKey(historyItem) !== key),
    ].slice(0, 100)

    savePlayHistoryThrottle()
    global.app_event.playHistoryUpdated([...playHistory])
  }
  playHistoryUpdatePromise = playHistoryUpdatePromise.then(update, update)
  return playHistoryUpdatePromise
}
...
export const getTempListMeta = async() => {
  return await getData<{ id: string, playlist?: LX.Player.TempPlaylistMeta }>(tempListMetaKey) ?? {
    id: '',
    playlist: undefined,
  }
}

export const saveTempListMeta = async(meta: { id: string, playlist?: LX.Player.TempPlaylistMeta }) => {
  return saveData(tempListMetaKey, meta)
}
