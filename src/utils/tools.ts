...
export const shareSonglist = (shareType: LX.ShareType, name: string, desc: string) => {
  const songlistName = name.trim()
  const content = [songlistName, desc.trim()].filter(Boolean).join('\n')
  switch (shareType) {
    case 'system':
      void shareText(global.i18n.t('share_card_title_songlist', { name: songlistName }), global.i18n.t('share_title_songlist'), content)
      break
    case 'clipboard':
...
export const cheatTip = async() => {
  ...
  return tipDialog({
    title: '谨防被骗提示',
    message: `1. 本项目无微信公众号之类的所谓「官方账号」，也未在小米、华为、vivo 等应用商店发布应用，商店内的「寻音」「洛雪音乐」相关的应用全部属于假冒应用，谨防被骗！\n
