...
import { addPlaylistHistoryFromPlayback } from '@/core/player/playlistHistory'
import {
  clearTempPlayeList,
  removeTempPlayList,
...
const handlePlay = async() => {
  ...
  if (global.lx.restorePlayInfo) {
    ...
    if (musicInfo) {
      ...
      addPlaylistHistoryFromPlayback(playMusicInfo as LX.Player.PlayMusicInfo)
 ...
 * 播放稍后播放队列内的歌曲
 */
export const playTempPlayList = async(index: number): Promise<void> => {
