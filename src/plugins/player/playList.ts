...
import { isMusicCollected } from '@/core/player/love'
...
const buildTracks = async(musicInfo: LX.Player.PlayMusic, url?: LX.Player.Track['url'], duration?: LX.Player.Track['duration']): Promise<LX.Player.Track[]> => {
  const mInfo = formatMusicInfo(musicInfo)
  const rating = await isMusicCollected(mInfo.id)
...
export const initTrackInfo = async(musicInfo: LX.Player.PlayMusic, mInfo: LX.Player.MusicInfo) => {
  const tracks = await buildTracks(musicInfo)
...
// console.log(tracks, time)
  const tracks = await buildTracks(musicInfo, url)
